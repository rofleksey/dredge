...
func TestHandler_Login_ok(t *testing.T) {
	...
	twSvc := twitch.New(twRepo, noopBroadcaster{}, testTwitchServiceConfig("c", "s"), obs)
	h := NewHandler(authSvc, settings.New(nil, obs), twSvc, nil, obs)
...
func TestHandler_Me_ok(t *testing.T) {
	...
	ctx := context.WithValue(context.WithValue(context.Background(), userIDCtxKey, uid), roleCtxKey, role)
